(function(ionic) {
  ...
  .directive('swipeCard', ['$timeout','$window', function($timeout,$window) {
    
    //Adding fake class at runtime
    var size = Math.floor($window.innerWidth * 95 / 100);
    var left = Math.floor(($window.innerWidth - size) / 2);
    var top = Math.floor(($window.innerHeight - size) / 2) - 88; //HACK: sub-header;

    return {
      restrict: 'E',
      template: '<div style="Z-Index:1000; position:fixed; width:' + size + 'px; height:' + size + 'px; left:' + left + 'px; top:' + top + 'px;" class="swipe-card" ng-transclude></div>',
      require: '^swipeCards',
      replace: true,
      transclude: true,
      scope: {
      ...
      compile: function(element, attr) {
        return function($scope, $element, $attr, swipeCards) {
          ...
          console.log($element.parent().parent()[0].getBoundingClientRect().top);
